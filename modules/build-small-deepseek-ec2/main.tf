...
resource "aws_instance" "small-deepseek-complete-builder" {
  ...
  root_block_device {
    volume_size = 360  # Root volume for OS and dependencies
    volume_type = "gp3"
  }
  ...
  user_data = <<-EOF
    ...
    [Service]
    ExecStart=/usr/local/bin/ollama run deepseek-r1:7b > /dev/null 2>&1
    Restart=always
    RestartSec=5
    StartLimitIntervalSec=500
    StartLimitBurst=5
    ...
    for model in "DeepSeek-R1-Distill-Qwen-32B:11436" \
             "DeepSeek-R1-Distill-Qwen-14B:11437"
    do
    ...
    sudo ufw allow $SSH_PORT/tcp || echo "UFW not installed, skipping..." >> /tmp/setup.log
...
resource "aws_ami_from_instance" "deepseek-complete-ami" {
  depends_on = [aws_instance.small-deepseek-complete-builder]
  name        = "${var.prepend-name}DeepSeek-Complete-AMI"
  description = "AMI with DeepSeek inference setup"
  source_instance_id = aws_instance.small-deepseek-complete-builder.id
